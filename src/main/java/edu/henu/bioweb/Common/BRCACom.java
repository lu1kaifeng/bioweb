...
public class BRCACom {
			...
			rs = stmt.executeQuery(strSql);
				...
				while (rs.next()) {
					...
					{	
						...
						else if(strSurvival.equals("12"))
						{
							strTime = rs.getString("DMS");
							strEvent = rs.getString("DMS_Event");
							strR = "";
							strR = res + "<-data.frame(DMS=c("+strTime+"),DMS_Event=c("+strEvent+"),genValue=c("+strValue+"))\n";
						}
					}
					else
					{
	...
	{	
		...
		if(strSql.equals("-1"))
		{
			...
			strTip = "Number of BRCA patients was less than 4, No meaningful output.";
            return strTip;
		}
        ...
        try {   
            ...
            if(i<4 && i>0)
            {            	
            	strTip = fun.GetTip(2, tip, strGen, "BRCA");
                return strTip;
            }
            else if(i==0)
            {
            	strTip = fun.GetTip(3, tip, strGen, "BRCA");
                return strTip;
            }
            else
            {
            ...
            re.eval("gmax<-gen");
            re.eval("gmax<-gmax[order(-gmax[,1]),]");
            ...
            re.eval("ggsurv$plot <- ggsurv$plot+ ggplot2::annotate(\"text\", x = gmax[1,1]*0.9, y = 0.05,label = \""+strCancer+"\", size = 5,fontface = 'italic')");
