...
public class CHOLCom {
	...
	public boolean isExistByGene(String strGene, String strGSE){
        ...
        try {
        	...
        	obj = (Object)ctx.lookup("jdbc/SSCHOL");
            ds = (DataSource)obj;
	...
	{	
		...
		if(strSql.equals("-1"))
		{
			strTip = fun.GetTip(3, tip, strGen, "CHOL");
            return strTip;
		}
        ...
        try {   
            ...
            if(i<4 && i>0)
            {            	
            	strTip = fun.GetTip(2, tip, strGen, "CHOL");
                return strTip;
            }
            else if(i==0)
            {
            ...
            else if(strSplit.equals("10"))
            {
            	re.eval("r1<-round(r*0.2)");
            	re.eval("if(gen[r1,3]==gen[r1+1,3]){suba<-gen[r1,3]-gen[r1-1,3];subb<-gen[r1+1,3]-gen[r1+2,3];if(abs(suba)>abs(subb)){gen.up<-gen[1:r1,];gen.low<-gen[(r1+1):r,]}else{gen.up<-gen[1:(r1+1),];gen.low<-gen[(r1+2):r,]}}else{gen.up<-gen[1:r1,];gen.low<-gen[(r1+1):r,]}");
            }
            ...
            if(strSplit.equals("1") || strSplit.equals("3") || strSplit.equals("4") || strSplit.equals("5") || strSplit.equals("6") || strSplit.equals("10"))
            {	
