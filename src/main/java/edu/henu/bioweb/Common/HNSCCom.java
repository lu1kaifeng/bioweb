...
public class HNSCCom {
	...
	{	
		...
		if(strSql.equals("-1"))
		{
			...
			strTip = fun.GetTip(3, tip, strGen, "HNSCC");//"Number of HNSCC patients was less than 4 in "+tip+", No meaningful output.";
            return strTip;
		}
        ...
        try {   
            ...
            if(i<4 && i>0)
            {            	
            	strTip = fun.GetTip(2, tip, strGen, "HNSCC");
                return strTip;
            }
            else if(i==0)
            {
            	strTip = fun.GetTip(3, tip, strGen, "HNSCC");
                return strTip;
            }
            else
            {
            ...
            String strSurv = fun.GetSurvival(strSurvival);
            ...
            {
            	re.eval("ggsurv<-ggsurvplot(gen.surv,data=gen,conf.int = TRUE,conf.int.style='step',conf.int.alpha=0.3,risk.table=TRUE,xlab=\"Months\",palette=c(\"red\",\"black\",\"#E7B800\",\"green\"),main=\"Survival curve\",font.main = c(16, \"bold\",\"darkblue\"),font.x = c(16, \"bold\", \"black\"),font.y = c(16, \"bold\", \"black\"),font.tickslab = c(16, \"plain\", \"black\"),legend.title=\" \", legend.labs = c(\""+strLab1+"\", \""+strLab2+"\",\""+strLab3+"\",\""+strLab4+"\"),legend = 'none')");
            	re.eval("ggsurv$table <- ggsurv$table + theme(axis.text.y = element_text(colour='black'))");
        	...
        	String strTiff = strPath.substring(0, index) + ".tiff";
        	re.eval("ggsave(file = \""+strTiff+"\", print(ggsurv))");
        }
